...
export const ChartsSection: React.FC<ChartsSectionProps> = ({ result }) => {
  ...
  const monthlyData = result.monthlyBreakdown.map((m) => ({
    name: `${m.month}월`,
    구독매출: Math.round(m.subscriptionRevenue / 10000),
    온보딩매출: Math.round(m.onboardingRevenue / 10000),
    누적브랜드: m.cumulativeBrands,
  }));
  ...
  return (
    <div className="card">
      ...
      <div style={{ marginBottom: '48px' }}>
        <h3 style={{ fontSize: '16px', marginBottom: '16px' }}>월별 매출 추이</h3>
